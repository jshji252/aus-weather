...
def get_header():
    def epocher(time_int):
        ...
        s=str(time_int)
        epoch=1000*(datetime.datetime(int(s[:4]),int(s[4:6]),int(s[6:8]),int(s[8:10]),int(s[10:12])) - datetime.datetime(1970,1,1,0,0)).total_seconds()
        print(s)
        print(epoch)
        return epoch
    ...
    try:
        ...
        datas = [{"date": epocher(data["aifstime_utc"]), "temp": data["apparent_t"], "sort_order": data["sort_order"]} for data in weather_data]
